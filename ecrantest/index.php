...
// Fonction pour générer un ID utilisateur aléatoire de 4 lettres
function generateUserID($length = 4) {
    return substr(str_shuffle('ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 0, $length);
}

// Gérer l'identifiant utilisateur via cookie
if (!isset($_COOKIE['user'])) {
    $userID = generateUserID();
    setcookie('user', $userID, time() + (365 * 24 * 3600)); // 1 an
    $_COOKIE['user'] = $userID;
}

$user = strtoupper(preg_replace('/[^A-Z]/', '', $_COOKIE['user']));
$user = substr($user, 0, 4); // sécurité : max 4 caractères

// Gestion de réinitialisation
if (isset($_GET['reset'])) {
...
$userDir = "users/$user/";
$slidesDir = $userDir . "slides/";
$orderFile = $userDir . "order.json";
...
<div class="container">
  <div class="text-end mb-3">
    <a href="?reset=1" class="btn btn-outline-danger btn-sm">
      Nouveau compte (actuel : <?= htmlspecialchars($user) ?>)
    </a>
  </div>
  ...
  <?php else: ?>
    <div class="card mb-4 p-3">
      ...
      <form method="post" enctype="multipart/form-data">
        <div class="mb-2"><input class="form-control" type="file" name="fichier" required></div>
